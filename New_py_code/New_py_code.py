...
new_py_name = pyautogui.prompt(
    text='What should the project be called?', title='New_py_Code', default='')
default_folder = read_config(file_path_config, "default", "folder")
projekt_folder = pyautogui.prompt(
    text='what is the destination folder?', title='New_py_Code', default=default_folder)
...
__funktion__temp = Read_File_Out(
    file_path + os.path.sep + "Templates" + os.path.sep+"__funktion__-temp.txt")
...
gitignor += Read_File_Out(file_path + os.path.sep +
                          "Templates" + os.path.sep + ".gitignor-temp.txt")
Create_File(f".gitignore", projekt_folder, gitignor)

requirements_content = Read_File_Out(
    file_path + os.path.sep + "Templates" + os.path.sep + "requirements-temp.txt")
Create_File(f"requirements.txt", projekt_folder, requirements_content)


TODO_md_txt_content = Read_File_Out(
    file_path + os.path.sep + "Templates" + os.path.sep + "TODO_md.txt")
Create_File(f"TODO.md", projekt_folder, TODO_md_txt_content)

test_py_temp = Read_File_Out(
    file_path + os.path.sep + "Templates" + os.path.sep + "test_py-temp.txt")
Create_File(f"test.py", test_folder, test_py_temp)

readme_gen(new_py_name, projekt_folder, Readme_top,
           readme_img_folder, file_path_config)
